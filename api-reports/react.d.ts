...
import { SubmissionIdFormat, BaseSubmissionMetadata, FormSchema, FormValues, StrictFormSubmission, FormPolicy, TranslationAdapter, AsyncTranslationAdapter, LocaleOption, TranslationReport, TranslationSlot, PopulateTranslationOptions, TranslationProgress, QuestionType, FormField, ChoiceOption, FormPage, DisplayCondition, JsonValue, SchemaIssue, FieldOption, Question, ValidationIssue, SubmissionGuard as SubmissionGuard$1, FormValue, QuizEvaluationResult, ValidationError, FormAcceptanceResult, FormTelemetryAdapter, FormTelemetryContextValue, FormInteractionEvent, TranslationStatus, CanonicalTranslationMetadata, FieldType, PollRuntimeAdapter, FormAnalytics, QuizQuestionResult, getContentModeDiagnostics, AnswerValidationResult, AuthoringApplyResult, AuthoringAssistantAdapter, AuthoringSuggestion, AuthoringPreview, AuthoringRequest, CreationAssistantAdapter, CreationMessage, SurveyCreationBrief, CreationQuickReply, CreationAssistantResponse, PollAccessContext, FormEngineTranslator, FormEngineMessages, TranslationWorkspaceCustomDictionary, TranslationMissingKeyEvent } from '@form-engine-ts/core';
export { FormInteractionEvent, FormSubmissionError, FormSubmissionSerializedError, FormTelemetryAdapter, QuestionType, SubmissionIdFormat, TranslationWorkspaceCustomDictionary } from '@form-engine-ts/core';
...
interface SubmissionReceiptStore {
    get(formId: string, formVersion: number, scope?: Pick<SubmissionReceiptQuery, "deckId" | "sessionId" | "userId" | "tenantId">): Promise<SubmissionReceipt | null>;
    ...
    remove(formId: string, formVersion: number, scope?: Pick<SubmissionReceiptQuery, "deckId" | "sessionId" | "userId" | "tenantId">): Promise<void>;
}
...
declare function submissionReceiptQueryKey(formId: string, formVersion: number, scope?: Pick<SubmissionReceiptQuery, "deckId" | "sessionId" | "userId" | "tenantId">): string;
...
interface SubmissionIdentityOptions<TMeta extends BaseSubmissionMetadata = BaseSubmissionMetadata> {
...
interface SubmissionIdentity<TMeta extends BaseSubmissionMetadata = BaseSubmissionMetadata> {
    readonly schema: FormSchema;
    readonly scope: SubmissionAttemptScope;
    readonly idFormat: SubmissionIdFormat;
    readonly attemptStore: SubmissionAttemptStore;
...
declare function createSubmissionIdentity<TMeta extends BaseSubmissionMetadata = BaseSubmissionMetadata>(options: SubmissionIdentityOptions<TMeta>): SubmissionIdentity<TMeta>;
...
type FieldMutationReason = "add" | "update" | "changeType" | "option";
...
declare function useFormBuilder(options: FormBuilderOptions): FormBuilderResult;
...
interface BuilderBasicSettingsSlotProps extends BuilderSlotBaseProps {
    readonly locale: string;
}
interface BuilderFieldEditorPreviewSlotProps extends BuilderSlotBaseProps {
    readonly field: FormField;
    readonly index: number;
    readonly totalFields: number;
    readonly onSelect: () => void;
}
interface BuilderFieldEditorBelowRequiredSlotProps extends BuilderSlotBaseProps {
    readonly field: FormField;
    readonly index: number;
}
...
type FormBuilderSectionVisibility = Partial<Record<FormBuilderSectionName, boolean>>;
...
interface TranslationComparisonColumnAppearance {
    readonly backgroundColor?: string;
    readonly borderColor?: string;
    readonly borderWidth?: number | string;
    readonly borderRadius?: number | string;
...
interface RespondentPrimitiveProps {
    readonly field: FormField;
    readonly id: string;
    readonly name: string;
...
interface RespondentTextInputProps extends RespondentPrimitiveProps {
...
interface RespondentTextAreaProps extends RespondentPrimitiveProps {
...
interface RespondentSelectProps extends RespondentPrimitiveProps {
    readonly value?: string | undefined;
    readonly options: readonly FieldOption[];
    readonly onChange: (value: string | undefined) => void;
}
interface RespondentCheckboxProps extends RespondentPrimitiveProps {
    readonly checked: boolean;
    readonly value?: string | undefined;
    readonly onChange: (checked: boolean) => void;
}
interface RespondentRadioProps extends RespondentPrimitiveProps {
    readonly checked: boolean;
    readonly value?: string;
    readonly onChange: (checked: boolean) => void;
    readonly onKeyDown?: ((event: KeyboardEvent<HTMLElement>) => void) | undefined;
}
interface RespondentRatingProps extends RespondentPrimitiveProps {
    readonly checked: boolean;
...
type RespondentButtonKind = "previous" | "next" | "submit" | "confirm" | "cancel" | "retry" | "draft-resume" | "draft-start-over" | "reset";
interface RespondentButtonProps {
...
interface ChoiceOptionSlotProps {
    readonly field: Question;
    readonly option: FieldOption;
    readonly inputId: string;
...
interface FormDraftResumeSlotProps {
...
interface FormRendererPresentationProps extends SubmissionProtectionProps {
    ...
    readonly pageTransition?: FormPageTransitionOptions;
...
type AuthoringAssistantStatus = "idle" | "generating" | "ready" | "applying" | "error";
type AuthoringAssistantError = {
    ...
    readonly cause: AuthoringApplyResult;
} | {
    readonly code: "stale_schema";
    readonly cause: AuthoringApplyResult;
};
interface UseAuthoringAssistantOptions {
    readonly schema: FormSchema;
...
interface UseAuthoringAssistantResult {
...
declare function useAuthoringAssistant(options: UseAuthoringAssistantOptions): UseAuthoringAssistantResult;

type CreationAssistantStatus = "idle" | "collecting" | "responding" | "ready" | "generating" | "reviewing" | "applying" | "completed" | "error";
type CreationAssistantErrorCode = "provider_unavailable" | "network_error" | "invalid_response" | "policy_violation" | "stale_schema" | "rate_limit" | "aborted";
interface CreationAssistantError {
    readonly code: CreationAssistantErrorCode;
    readonly cause?: unknown;
}
interface UseFormCreationAssistantOptions {
...
interface UseFormCreationAssistantResult {
...
declare function useFormCreationAssistant(options: UseFormCreationAssistantOptions): UseFormCreationAssistantResult;
...
declare const FormEngineI18nProviderScopeContext: react.Context<boolean>;
