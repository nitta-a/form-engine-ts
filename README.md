...
The latest release is **v4.7.0** (2026-08-29). All public packages are currently published at version `4.7.0`.
This release adds migration context for translation metadata and an extensible locale-validation pipeline to the Core and React workspace APIs.
See [RELEASE_NOTES.md](RELEASE_NOTES.md) for the complete history and the
[GitHub release](https://github.com/nitta-a/form-engine-ts/releases/tag/v4.7.0).
...
最新版は **v4.7.0**（2026-08-29）です。公開パッケージはすべてバージョン `4.7.0` で公開されています。
本リリースでは、翻訳メタデータ移行コンテキストと、Core／ReactワークスペースAPIの拡張可能なロケール検証パイプラインを追加しました。
全更新履歴は[RELEASE_NOTES.md](RELEASE_NOTES.md) と
[GitHub Release](https://github.com/nitta-a/form-engine-ts/releases/tag/v4.7.0) を参照してください。
