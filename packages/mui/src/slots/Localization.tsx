...
import { TranslationWorkspace } from "../workspace/TranslationWorkspace";
...
export function createMuiLocalizationSlot(options?: MuiAdapterOptions): ComponentType<BuilderLocalizationSlotProps> {
  return function MuiLocalization(props) {
    const resolved = useResolvedMuiAdapterOptions(options);
    if (resolved.localization?.mode === "inline-workspace") {
      return <MuiInlineLocalizationWorkspace {...props} options={options} />;
    }
    return <MuiStandardLocalization {...props} options={options} />;
  };
}

function MuiInlineLocalizationWorkspace({
  schema,
  currentLocale,
  policy,
  readOnly,
  onChange,
  options
}: BuilderLocalizationSlotProps & { readonly options: MuiAdapterOptions | undefined }) {
  const resolved = useResolvedMuiAdapterOptions(options);
  const workspaceOptions = resolved.localization?.workspaceOptions;
  return (
...
function MuiStandardLocalization({
  schema,
  currentLocale,
