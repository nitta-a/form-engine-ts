import type { FormEngineTranslationKey } from "../keys";

export const JA_COMPARISON_MESSAGES: Readonly<Record<string, string>> = Object.freeze({
  "workspace.comparison.title": "翻訳比較ワークスペース",
  "workspace.comparison.emptyStateTitle": "翻訳を始める",
  "workspace.comparison.sourceHeader": "原文 ({{locale}})",
  "workspace.comparison.targetHeader": "訳文 ({{locale}})",
  "workspace.comparison.property.title": "タイトル",
  "workspace.comparison.property.description": "説明文",
  "workspace.comparison.property.label": "選択肢ラベル",
  "workspace.comparison.property.completionMessage": "送信完了メッセージ",
  "workspace.comparison.property.closedMessage": "受付終了メッセージ",
  "workspace.comparison.property.notYetOpenMessage": "受付開始前メッセージ",
  "workspace.comparison.emptySource": "（原文なし）",
  "workspace.comparison.staleWarning": "原文が変更されています"
});

export const JA_MESSAGES: Readonly<Record<FormEngineTranslationKey, string>> = Object.freeze({
  "authoring.prompt.label": "フォームの変更内容を入力",
  "authoring.prompt.submit": "AIに依頼",
  "authoring.preview.selectAll": "すべて選択",
  "authoring.preview.apply": "選択した変更を適用",
  "authoring.preview.reject": "却下",
  "authoring.preview.before": "変更前",
  "authoring.preview.after": "変更後",
  "authoring.preview.invalid": "無効",
  "authoring.preview.label": "AI提案のプレビュー",
  "authoring.preview.property.type": "質問形式",
  "authoring.preview.property.title": "質問文",
  "authoring.preview.property.description": "説明",
  "authoring.preview.property.required": "必須回答",
  "authoring.preview.property.option": "選択肢",
  "authoring.preview.property.completionMessage": "完了メッセージ",
  "authoring.preview.property.submitLabel": "送信ラベル",
  "authoring.preview.value.required": "必須",
  "authoring.preview.value.optional": "任意",
  "authoring.preview.value.none": "—",
  "authoring.preview.value.enabled": "有効",
  "authoring.preview.value.disabled": "無効",
  "authoring.action.ai": "✨ AI",
  "authoring.action.rewrite": "質問文を改善",
  "authoring.action.shorten": "短くする",
  "authoring.action.generateOptions": "選択肢を生成",
  "authoring.operation.addField": "質問を追加",
  "authoring.operation.updateField": "質問を変更",
  "authoring.operation.updateForm": "フォーム設定を変更",
  "authoring.operation.addOption": "選択肢を追加",
  "authoring.operation.updateOption": "選択肢を変更",
  "builder.content.mode": "フォーム種別",
  "builder.content.survey": "アンケート",
  "builder.content.poll": "投票",
  "builder.content.quiz": "クイズ",
  "builder.content.pollSettings": "投票設定",
  "builder.content.quizSettings": "クイズ設定",
  "builder.content.resultVisibility": "結果公開タイミング",
  "builder.content.after_submit": "送信後",
  "builder.content.always": "常に公開",
  "builder.content.closed_only": "締切後",
  "builder.content.private": "非公開",
  "builder.content.strictOneVotePerUser": "一人一票",
  "builder.content.showExplanation": "解説表示タイミング",
  "builder.content.immediate": "選択直後",
  "builder.content.enablePassingScore": "合格ラインを設定",
  "builder.content.passingScore": "合格ライン点数",
  "builder.content.correctAnswer": "正解",
  "builder.content.explanation": "解説",
  "builder.content.points": "配点",
  "builder.content.validationTitle": "コンテンツ設定を修正してください",
  "builder.content.validation.poll_field_count": "投票には質問が1問必要です。",
  "builder.content.validation.quiz_field_count": "クイズには1問以上の質問が必要です。",
  "builder.content.validation.unsupported_field_type": "このフォーム種別では使用できない質問形式です。",
  "builder.content.validation.options_minimum": "選択肢が2件以上必要です。",
  "builder.content.validation.correct_option_missing": "正解を選択してください。",
  "builder.content.validation.points_type": "配点には数値を指定してください。",
  "builder.content.validation.explanation_type": "解説には文字列を指定してください。",
  "builder.content.validation.points_range": "配点には0以上の有限値を指定してください。",
  "builder.content.validation.poll_result_visibility": "有効な結果公開タイミングを選択してください。",
  "builder.content.validation.poll_strict_one_vote": "一人一票の設定には真偽値を指定してください。",
  "builder.content.validation.quiz_explanation_timing": "有効な解説表示タイミングを選択してください。",
  "builder.content.validation.quiz_passing_score": "合格点は0から合計点の範囲で指定してください。",
  "content.results.totalScore": "合計得点",
  "content.results.passed": "合格",
  "content.results.notPassed": "不合格",
  "content.results.correct": "正解",
  "content.results.incorrect": "不正解",
  "content.results.correctOption": "正解の選択肢",
  "content.results.pollResults": "投票結果",
  "content.results.votes": "票",
  "content.results.loading": "結果を読み込んでいます",
  "content.results.retry": "結果を再取得",
  "content.results.loadError": "結果を取得できませんでした。",
  "content.results.invalidQuiz": "クイズの設定が正しくありません。",
  "content.results.share": "結果を共有",
  "content.results.shared": "共有しました",
  "content.results.copied": "コピーしました",
  "content.results.shareFailed": "共有できませんでした",
  "builder.formTitle": "フォームのタイトル",
  "builder.formDescription": "フォームの説明",
  "builder.completionMessage": "送信完了メッセージ",
  "builder.structure": "フォーム構成",
  "builder.page": "ページ",
  "builder.questions": "質問",
  "builder.expand": "展開",
  "builder.collapse": "折りたたむ",
  "builder.validationProblems": "修正が必要な項目",
  "builder.fixIssue": "修正",
  "builder.addQuestion": "質問を追加",
  "builder.actions.addField": "質問を追加",
  "builder.actions.deleteField": "削除",
  "builder.actions.moveUp": "上へ移動",
  "builder.actions.moveDown": "下へ移動",
  "builder.actions.add": "追加",
  "builder.actions.delete": "削除",
  "builder.actions.edit": "編集",
  "builder.actions.settings": "設定",
  "builder.actions.translate": "翻訳",
  "builder.actions.close": "閉じる",
  "builder.actions.dragHandle": "並べ替え",
  "builder.fields.selectType": "質問形式を選択",
  "builder.fields.typeText": "一行テキスト",
  "builder.fields.typeTextarea": "長文テキスト",
  "builder.fields.typeNumber": "数値",
  "builder.fields.typeRadio": "単一選択 (ラジオボタン)",
  "builder.fields.typeCheckbox": "チェックボックス",
  "builder.fields.typeSelect": "ドロップダウン",
  "builder.fields.typeRating": "評価スケール",
  "builder.fields.typeMultiSelect": "複数選択",
  "builder.fields.typeDate": "日付",
  "builder.fields.typeTime": "時刻",
  "builder.fields.typeEmail": "メールアドレス",
  "builder.fields.typeTel": "電話番号",
  "builder.fields.typeUrl": "URL",
  "builder.fieldType.text": "一行テキスト",
  "builder.fieldType.textarea": "長文テキスト",
  "builder.fieldType.number": "数値",
  "builder.fieldType.radio": "単一選択",
  "builder.fieldType.checkbox": "チェックボックス",
  "builder.fieldType.select": "ドロップダウン",
  "builder.fieldType.rating": "評価スケール",
  "builder.fieldType.multi-select": "複数選択",
  "builder.fieldType.date": "日付",
  "builder.fieldType.time": "時刻",
  "builder.fieldType.email": "メールアドレス",
  "builder.fieldType.tel": "電話番号",
  "builder.fieldType.url": "URL",
  "builder.fieldTypeDescription.text": "一行の回答",
  "builder.fieldTypeDescription.textarea": "長文の回答",
  "builder.fieldTypeDescription.number": "数値の回答",
  "builder.fieldTypeDescription.radio": "単一選択の回答",
  "builder.fieldTypeDescription.checkbox": "はい / いいえの回答",
  "builder.fieldTypeDescription.select": "ドロップダウン形式の回答",
  "builder.fieldTypeDescription.rating": "評価スケールの回答",
  "builder.fieldTypeDescription.multi-select": "複数選択の回答",
  "builder.fieldTypeDescription.date": "日付の回答",
  "builder.fieldTypeDescription.time": "時刻の回答",
  "builder.fieldTypeDescription.email": "メールアドレス",
  "builder.fieldTypeDescription.tel": "電話番号",
  "builder.fieldTypeDescription.url": "URL",
  "builder.fieldCategory.text": "テキスト",
  "builder.fieldCategory.choice": "選択式",
  "builder.fieldCategory.number": "数値",
  "builder.fieldCategory.advanced": "詳細設定",
  "builder.required": "必須項目にする",
  "builder.options": "選択肢一覧",
  "builder.shuffleOptions": "回答者ごとに選択肢を並べ替える",
  "builder.optionDisplayOrder": "選択肢の表示順",
  "builder.pinOption": "この選択肢を固定する",
  "builder.optionTextInput": "補足テキストを許可する",
  "builder.openAt": "受付開始日時 (ISO形式)",
  "builder.closeAt": "受付終了日時 (ISO形式)",
  "builder.maxResponses": "回答上限",
  "builder.closedMessage": "受付終了メッセージ",
  "builder.notYetOpenMessage": "受付開始前メッセージ",
  "builder.honeypotFieldId": "スパム対策用の非表示項目ID",
  "builder.localization.title": "多言語設定",
  "builder.localization": "多言語設定",
  "builder.localization.addLocale": "言語を追加",
  "builder.localization.selectLocaleToAdd": "追加する言語を選択",
  "builder.localization.defaultLocale": "デフォルト言語 (元言語)",
  "builder.localization.translateAll": "一括自動翻訳",
  "builder.localization.noLocalesConfigured": "翻訳言語はまだ設定されていません。",
  "builder.localization.localesConfiguredSummary": "{{count}}言語の翻訳が設定されています。",
  "builder.localization.allLocalesAdded": "すべての候補言語が追加済みです",
  "builder.localization.maxLocalesReached": "登録可能な最大言語数 ({{max}}) に達しました",
  "builder.submissionSettings.title": "送信設定",
  "builder.submissionSettings.showConfirmation": "送信前に回答確認画面を表示する",
  "builder.submissionSettings.renderMode": "確認画面の表示形式",
  "builder.formBuilder": "フォームビルダー",
  "builder.basicSettings": "基本設定",
  "builder.description": "説明",
  "builder.moveUp": "{{title}}を上へ移動",
  "builder.moveDown": "{{title}}を下へ移動",
  "builder.delete": "{{title}}を削除",
  "builder.deleteAction": "削除",
  "builder.questionTitle": "質問文",
  "builder.questionTitlePlaceholder": "例: 改善できる点を教えてください",
  "builder.newQuestionTitle": "新しい質問",
  "builder.type": "形式",
  "builder.minimum": "最小値",
  "builder.maximum": "最大値",
  "builder.minimumLength": "最小文字数",
  "builder.maximumLength": "最大文字数",
  "builder.pattern": "形式パターン",
  "builder.step": "刻み幅",
  "builder.optionLabel": "選択肢 {{index}}",
  "builder.optionLabelPlaceholder": "例: とても満足",
  "builder.newOptionLabel": "選択肢 {{index}}",
  "builder.remove": "削除",
  "builder.addOption": "選択肢を追加",
  "builder.displayCondition": "表示条件",
  "builder.alwaysVisible": "常に表示",
  "builder.conditionOperator": "条件演算子",
  "builder.conditionValue": "条件値",
  "builder.conditionTrue": "はい",
  "builder.conditionFalse": "いいえ",
  "builder.pages": "ページ管理",
  "builder.enablePages": "複数ページを有効にする",
  "builder.addPage": "ページを追加",
  "builder.splitPage": "質問を新しいページへ分ける",
  "builder.newPage": "新しいページ",
  "builder.pageTitle": "ページタイトル",
  "builder.pageDescription": "ページの説明",
  "builder.pageQuestion": "新しいページへ移動する質問",
  "builder.pageQuestionToMove": "新しいページへ移動する質問",
  "builder.noPageQuestions": "各ページには最低1問が必要です。先に質問を追加してください。",
  "builder.pageDeleteMoves": "このページを削除すると、質問は隣接するページへ移動します。",
  "builder.pageDeleteLast": "最後のページを削除するとページ分割を解除し、質問は残ります。",
  "builder.questionPage": "ページ",
  "builder.pageCondition": "ページの表示条件",
  "builder.unassigned": "未割り当て",
  "builder.defaultLocale": "デフォルト言語",
  "builder.supportedLocales": "対応言語",
  "builder.addLocale": "言語を追加",
  "builder.editLocale": "言語を編集",
  "builder.autoTranslate": "すべて翻訳",
  "builder.translating": "翻訳中…",
  "builder.translationLocale": "翻訳言語",
  "builder.selectLocale": "翻訳を編集する言語を選択してください。",
  "builder.selectLocaleToAdd": "追加する言語を選択",
  "builder.translation": "{{locale}}翻訳",
  "builder.translatedFormTitle": "翻訳後のフォームタイトル",
  "builder.translatedFormDescription": "翻訳後のフォーム説明",
  "builder.translatedCompletionMessage": "翻訳後の送信完了メッセージ",
  "builder.translatedQuestionTitle": "翻訳後の質問文",
  "builder.translatedDescription": "翻訳後の説明",
  "builder.translationUnavailable": "自動翻訳には非同期翻訳アダプターを設定してください。",
  "builder.operator.equals": "等しい",
  "builder.operator.not_equals": "等しくない",
  "builder.operator.contains": "含む",
  "builder.operator.not_empty": "空ではない",
  "builder.showConfirmationBeforeSubmit": "送信前に確認画面を表示する",
  "builder.confirmationRenderMode": "確認画面の表示形式",
  "renderer.submitButton": "送信する",
  "renderer.submittingButton": "送信中...",
  "renderer.retryButton": "再送信する",
  "renderer.requiredField": "この項目は必須です。",
  "renderer.validationSummary": "{{count}}件の入力エラーがあります。",
  "renderer.validationSummaryPlural": "{{count}}件の入力エラーがあります。",
  "renderer.alreadySubmittedTitle": "回答済みです",
  "renderer.alreadySubmittedMessage": "このアンケートにはすでに回答しています。",
  "renderer.progressLabel": "進捗",
  "renderer.remainingQuestions": "残り{{count}}問",
  "renderer.serverErrorSummary": "送信に失敗しました。内容をご確認の上、再度お試しください。",
  "renderer.confirmSensitiveDataTitle": "個人情報が含まれている可能性があります",
  "renderer.confirmSensitiveDataMessage":
    "以下の項目に個人情報とみられる記述があります。このまま送信してもよろしいですか？",
  "renderer.confirmButton": "このまま送信",
  "renderer.cancelButton": "修正する",
  "renderer.draftResumeTitle": "回答を続ける",
  "renderer.draftResumeMessage": "このブラウザーに保存された未送信の回答があります。",
  "renderer.draftResumeContinue": "続きから回答する",
  "renderer.draftResumeStartOver": "最初から回答する",
  "renderer.draftResumeEnabled": "この端末に回答を7日間保存する",
  "renderer.draftResumeDisabled": "この端末に回答を保存しない",
  "renderer.draftSaved": "この端末に一時保存しました",
  "renderer.draftSaveFailed": "この端末に回答を保存できませんでした。",
  "renderer.draftDeleteFailed": "保存された回答を削除できませんでした。",
  "form.submit": "送信する",
  "form.submitting": "送信中...",
  "form.back": "戻る",
  "form.next": "次へ",
  "form.step": "{{current}} / {{total}}",
  "form.draftRestored": "下書きを復元しました",
  "form.submissionBlocked": "個人情報が検出されたため送信できません。",
  "form.confirmSensitiveData": "個人情報が含まれている可能性があります。送信前に確認してください。",
  "form.confirmSubmission": "このまま送信",
  "form.cancelSubmission": "修正する",
  "form.yes": "はい",
  "form.no": "いいえ",
  "form.alreadySubmitted": "回答済みです",
  "form.submitAnother": "別の回答を送信",
  "form.closed": "このフォームの受付は終了しています。",
  "form.notYetOpen": "このフォームはまだ受付を開始していません。",
  "form.responseLimitReached": "このフォームは回答上限に達しています。",
  "form.optionText": "{{option}}の補足（任意）",
  "validation.required": "この項目は必須です。",
  "validation.invalidOption": "有効な選択肢を選択してください。",
  "validation.invalidType": "有効な値を入力してください。",
  "validation.max": "値が大きすぎます。",
  "validation.maxLength": "入力が長すぎます。",
  "validation.maxSelections": "選択数を減らしてください。",
  "validation.min": "値が小さすぎます。",
  "validation.minLength": "入力が短すぎます。",
  "validation.minSelections": "さらに選択してください。",
  "validation.pattern": "入力形式が正しくありません。",
  "validation.invalidFormat": "入力形式が正しくありません。",
  "validation.sensitiveData": "個人情報が含まれている可能性があります。",
  "validation.step": "正しい刻み幅の値を入力してください。",
  "validation.unknownField": "この項目は認識されません。",
  "workspace.title": "多言語翻訳ワークスペース",
  "workspace.status.missing": "未翻訳",
  "workspace.status.translated": "翻訳済み",
  "workspace.status.stale": "原文更新あり",
  "workspace.status.manual": "手動編集済み",
  "workspace.status.manualStale": "手動編集済み (原文更新あり)",
  "workspace.errors.localeNotAllowed": "指定された言語「{{locale}}」は設定できません。",
  "workspace.errors.maxLocalesExceeded": "登録可能な最大言語数 ({{max}}) に達しました。",
  "workspace.errors.readOnly": "読み取り専用モードのため変更できません。",
  "workspace.errors.adapterNotConfigured": "翻訳アダプターが設定されていません。",
  "workspace.errors.translationFailed": "翻訳処理に失敗しました。",
  "workspace.errors.localeAlreadyExists": "言語「{{locale}}」はすでに設定されています。",
  "workspace.errors.sourceLocale": "言語「{{locale}}」は原文言語のため、翻訳言語には追加できません。",
  "workspace.errors.invalidLocale": "言語「{{locale}}」は有効な言語タグではありません。",
  "workspace.errors.targetLocaleMissing": "翻訳先の言語を選択してください。",
  "workspace.errors.partialFailure": "{{succeeded}}件成功、{{failed}}件失敗で翻訳が完了しました。",
  "workspace.errors.cancelled": "翻訳をキャンセルしました。",
  "workspace.header.title": "多言語翻訳ワークスペース",
  "workspace.header.sourceLocale": "元言語",
  "workspace.header.targetLocale": "翻訳言語",
  "workspace.header.addLocale": "翻訳言語を追加",
  "workspace.header.removeLocale": "翻訳言語を削除",
  "workspace.header.translateAll": "一括自動翻訳",
  "workspace.header.cancel": "翻訳をキャンセル",
  "workspace.header.retry": "翻訳を再試行",
  "workspace.header.progress": "{{translated}}/{{total}} 翻訳済み ({{percent}}%)",
  "workspace.header.batchProgress":
    "{{completed}}/{{total}}件処理済み ({{percent}}%)、成功 {{succeeded}}件、失敗 {{failed}}件",
  "workspace.slot.sourceText": "原文",
  "workspace.slot.translatedText": "翻訳文",
  "workspace.slot.translateSingle": "この項目を翻訳",
  "workspace.slot.revertManual": "手動翻訳を元に戻す",
  "workspace.confirm.removeLocaleTitle": "言語を削除しますか？",
  "workspace.confirm.removeLocaleMessage": "{{locale}} とその翻訳を削除します。",
  "workspace.confirm.removeLocaleTranslatedCount": "{{count}}件の翻訳も削除されます。",
  "workspace.confirm.cancel": "キャンセル",
  "workspace.confirm.remove": "削除",
  "workspace.empty.noTargetLocales": "翻訳先の言語が設定されていません。",
  "workspace.empty.noSlotsToTranslate": "この言語には翻訳項目がありません。",
  "workspace.comparison.title": "翻訳比較ワークスペース",
  "workspace.comparison.emptyStateTitle": "翻訳を始める",
  "workspace.comparison.sourceHeader": "原文 ({{locale}})",
  "workspace.comparison.targetHeader": "訳文 ({{locale}})",
  "workspace.comparison.property.title": "タイトル",
  "workspace.comparison.property.description": "説明文",
  "workspace.comparison.property.label": "選択肢ラベル",
  "workspace.comparison.property.completionMessage": "送信完了メッセージ",
  "workspace.comparison.property.closedMessage": "受付終了メッセージ",
  "workspace.comparison.property.notYetOpenMessage": "受付開始前メッセージ",
  "workspace.comparison.emptySource": "（原文なし）",
  "workspace.comparison.staleWarning": "原文が変更されています",
  "workspace.comparison.placeholder.title": "{{targetLocale}}語のタイトルを入力",
  "workspace.comparison.placeholder.completionMessage": "{{targetLocale}}語の送信完了メッセージを入力",
  "workspace.comparison.placeholder.question": "{{targetLocale}}語の質問を入力",
  "workspace.comparison.placeholder.option": "{{targetLocale}}語の選択肢を入力",
  "workspace.comparison.nodeKind.form": "フォーム",
  "workspace.comparison.nodeKind.page": "ページ",
  "workspace.comparison.nodeKind.field": "項目",
  "workspace.comparison.nodeKind.option": "選択肢"
});
